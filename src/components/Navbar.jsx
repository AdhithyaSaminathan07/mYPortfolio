  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 py-6 ${scrolled ? 'bg-bg-dark/80 backdrop-blur-md py-4 shadow-lg' : ''}`}>
      <div className="max-w-7xl mx-auto px-8 flex justify-between items-center">
        ...
        <div className={`md:flex gap-8 items-center ${isOpen ? 'absolute top-full left-0 w-full bg-bg-dark/95 flex flex-col py-8 gap-6 shadow-xl border-b border-primary/20' : 'hidden'}`}>
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              onClick={() => setIsOpen(false)}
              className="font-medium relative group hover:text-primary transition-colors duration-300"
            >
              {link.name}
        ...
        <div className="md:hidden flex flex-col gap-1.5 cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
